...
error_messages! {
   ...
   INVALID_RULE_WHEN_CONTAINS_DISJUNCTION = 31: "Rule '{}' 'when' contains a disjunction.",
   INVALID_RULE_THEN = 32: "Rule '{}' 'then' '{}': must be exactly one attribute ownership, or exactly one relation.",
   INVALID_RULE_THEN_HAS = 33: "Rule '{}' 'then' '{}' tries to assign type '{}' to variable '{}', but this variable already had a type assigned by the rule 'when'. Try omitting this type assignment.",
   INVALID_RULE_THEN_VARIABLES = 34: "Rule '{}' 'then' variables must be present in rule 'when'.",
   INVALID_RULE_THEN_ROLES = 35: "Rule '{}' 'then' '{}' must specify all role types explicitly or by using a variable.",
   REDUNDANT_NESTED_NEGATION = 36: "Invalid query containing redundant nested negations.",
   VARIABLE_NOT_SORTED = 37: "Variable '{}' does not exist in the sorting clause.",
   INVALID_SORTING_ORDER = 38: "Invalid sorting order '{}'. Valid options: '{}' or '{}'.",
   INVALID_COUNT_VARIABLE_ARGUMENT = 39: "Aggregate COUNT does not accept a Variable.",
   ILLEGAL_GRAMMAR = 40: "Illegal grammar: '{}'",
   ILLEGAL_CHAR_IN_LABEL = 41: "'{}' is not a valid Type label. Type labels must start with a letter, and may contain only letters, numbers, '-' and '_'.",
}
