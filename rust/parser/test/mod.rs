...
//     let parsed = parse_query(query).unwrap();
//     let expected =
//         typeql_match!(cvar("f").sub("parenthood").relates(("father", "parent")).relates(("son", "child")));
...
//     let parsed = parse_query(query);
//     assert!(parsed.is_err());
//     let built = typeql_match!(cvar(()).isa("person"));
//     assert!(built.is_err());
// }
...
$x like "\d";"#;
    let parsed = parse_query(query).unwrap();
    let expected = (); // typeql_match!(cvar("x").like(r"\d"));
    assert_valid_eq_repr!(expected, parsed, query);
}
...
$x like "\"";"#;
    let parsed = parse_query(query).unwrap();
    let expected = (); // typeql_match!(cvar("x").like(r#"\""#));
    assert_valid_eq_repr!(expected, parsed, query);
}
...
//     let var = "人";
//     let expected = typeql_match!(cvar(var).isa("person"));
...
// fn test_utf8_label() {
//     let label = "人";
//     let expected = typeql_match!(cvar("x").isa(label));
//     assert!(expected.is_ok());
// }
...
// fn test_utf8_value() {
//     let value = "人";
//     let expected = typeql_match!(cvar("x").isa("person").has(("name", value)));
//     assert!(expected.is_ok());
// }
