...
count;"#;
    ...
    let expected = typeql_match!(rel("x").rel("y").isa("friendship")).get_fixed([cvar("x"), cvar("y")]).count();
...
group $x; count;"#;
    ...
    let expected =
        typeql_match!(rel("x").rel("y").isa("friendship")).get_fixed([cvar("x"), cvar("y")]).group(cvar("x")).count();
