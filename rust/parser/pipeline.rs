...
use super::{
    ...
    visit_assignment_var, visit_var, visit_var_or_propagate, visit_vars,
};
use crate::{
    TypeRef, TypeRefAny,
    common::{
    ...
    query::{
        ...
        pipeline::{
            ...
            stage::{
                ...
                delete::{Deletable, DeletableKind, WriteCondition},
                fetch::FetchSome,
                given::Given,
...
fn visit_fetch_attribute(node: Node<'_>) -> FetchAttribute {
    ...
    let span = node.span();
    let mut children = node.into_children();
    let owner = visit_var_or_propagate(children.consume_expected(Rule::var_or_propagate));
...
fn visit_fetch_object_body(node: Node<'_>) -> FetchObjectBody {
    ...
    match child.as_rule() {
        ...
        Rule::fetch_attributes_all => {
            let var = visit_var_or_propagate(child.into_children().consume_expected(Rule::var_or_propagate));
...
fn visit_operator_reduce(node: Node<'_>) -> Reduce {
    ...
    while let Some(child) = children.try_consume_any() {
        match child.as_rule() {
            ...
            Rule::GROUPBY => {
                debug_assert!(!reduce_assignments.is_empty());
...
pub(super) fn visit_reduce_assign(node: Node<'_>) -> ReduceAssign {
    ...
    let mut children = node.into_children();
    let variable = visit_assignment_var(children.consume_expected(Rule::assignment_var));
